...
{{$block := index .Blocks .BlockIndex}}{{$blocks := .Blocks}}{{$compositeFB := $block.CompositeFB}}{{$tcrestUsingSPM := .TcrestUsingSPM}}{{$tcrestSmartSPM := .TcrestSmartSPM}}{{$eventQueue := .EventQueue}}
			...
			{{range $withIndex, $with := $event.With}}
				...
				{{if and $source $varDef}}
					{{if $varDef.GetArraySize}}
						...
						{{end}}
				{{else}}
					me->{{$child.Name}}.{{$with.Var}} = me->{{$source}};
				{{end}}
			{{end}}
		{{end}}
	} {{end}}{{end}}{{end}}
