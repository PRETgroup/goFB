...
{{if algorithmNeedsCvodeInit $alg}}//AUTOGENERATED CVODE CODE: see {{$block.Name}}_{{$alg.Name}}_cvode_init()
	{{$block.Name}}_{{$alg.Name}}_cvode_init(me);
{{else if algorithmNeedsCvode $alg}}//AUTOGENERATED CVODE CODE: this is a cvode algorithm, so is defined elsewhere
	{{$block.Name}}_{{$alg.Name}}_f_tick(me);
