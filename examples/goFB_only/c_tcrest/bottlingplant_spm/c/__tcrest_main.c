...
void task(FlexPRET_t _SPM * c);

int main() {
	printf("bottlingplant_spm tcrest4 startup.\n");
	printf("sizes: %lu, %lu, %lu, %lu\n", sizeof(FlexPRET_t), sizeof(FlexPRET_t), sizeof(FlexPRET_t), sizeof(FlexPRET_t));
	mp_init();
...
void __attribute__ ((noinline)) timed_task(FlexPRET_t _SPM * c) {
