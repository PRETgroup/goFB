...
// This file represents the implementation of the ODE / Hybrid Automata Function Block for Gate
#include "FB_Gate.h"


/* Gate_preinit() is required to be called to 
 * initialise an instance of Gate. 
...
/* Gate_init() is required to be called to 
 * set up an instance of Gate. 
...
void Gate_xPrimeEqX(Gate_t *me, CVRhsFn ode_f, CVRootFn ode_g) {
...
void Gate_xPrimeEq10(Gate_t *me, CVRhsFn ode_f, CVRootFn ode_g) {
	//AUTOGENERATED CODE: this algorithm specifies the initialization of a CVODE SUNDIALS solver and was parsed from the algorithm's text field
	int flag;
...
static int Gate_xDotInDown_ode_f(realtype t, N_Vector ode_solution, N_Vector ode_solution_dot, void *user_data) {
	//AUTOGENERATED CODE: this function represents the ODE in Gate's xDotInDown algorithm
	//it is in a format that CVODE can process.
	Gate_t *me = (Gate_t*)user_data;
	
	NV_Ith_S(ode_solution_dot, 0) = (1 - NV_Ith_S(ode_solution, 0)) / 2;
	
	return 0;
}

static int Gate_xDotInDown_ode_g(realtype t, N_Vector ode_solution, realtype *gout, void *user_data) {
	//AUTOGENERATED CODE: this function represents the roots of the ODE in Gate's xDotInDown algorithm
	//it is in a format that CVODE can process.
...
int Gate_xDotInDown(Gate_t *me) {
	//AUTOGENERATED CODE: this function manages iterations of the ODE in Gate's xDotInDown algorithm
...
static int Gate_xDotInUp_ode_f(realtype t, N_Vector ode_solution, N_Vector ode_solution_dot, void *user_data) {
	//AUTOGENERATED CODE: this function represents the ODE in Gate's xDotInUp algorithm
	//it is in a format that CVODE can process.
	Gate_t *me = (Gate_t*)user_data;
	
	NV_Ith_S(ode_solution_dot, 0) = (10 - NV_Ith_S(ode_solution, 0)) / 2;
	
	return 0;
}

static int Gate_xDotInUp_ode_g(realtype t, N_Vector ode_solution, realtype *gout, void *user_data) {
	//AUTOGENERATED CODE: this function represents the roots of the ODE in Gate's xDotInUp algorithm
	//it is in a format that CVODE can process.
...
//return 1 if we found a root
int Gate_xDotInUp(Gate_t *me) {
	//AUTOGENERATED CODE: this function manages iterations of the ODE in Gate's xDotInUp algorithm
	//It calls the CVODE solver and sets the outputs of the HA FB.
	if(me->solveInProgress == 0) {
		me->Tnext += me->DeltaTime;
		me->solveInProgress = 1; //solveInProgress is used to mark if we are currently solving
...
/* Gate_run() executes a single tick of an
 * instance of Gate according to synchronous semantics.
...
repeat: 	//when we have had a mid-tick transition, we want to start the run again 
	...
	if(me->_trigger == false) {
		...
		case STATE_Gate_State2E0:
			if(me->inputEvents.event.DOWN) {
				...
				me->_state = STATE_Gate_State2;
				me->_trigger = true;
			} else if(me->inputEvents.event.DOWN) {
	...
	if(me->_trigger == true) {
		...
		case STATE_Gate_State2E0:
			...
			//init all ode algorithms that this state feeds into
			Gate_xPrimeEqX(me, Gate_xDotInUp_ode_f, Gate_xDotInUp_ode_g);
			me->outputEvents.event.update_g = 1;
		...
		case STATE_Gate_State1E1:
			...
			//init all ode algorithms that this state feeds into
			Gate_xPrimeEqX(me, Gate_xDotInDown_ode_f, Gate_xDotInDown_ode_g);
			me->outputEvents.event.update_g = 1;
