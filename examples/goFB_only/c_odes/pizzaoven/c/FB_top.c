...
int top_preinit(top_t  *me) {
	...
	if(oven_preinit(&me->o) != 0) {
		return 1;
	}
	if(conveyor_preinit(&me->c) != 0) {
		return 1;
	}
...
int top_init(top_t  *me) {
	...
	me->contr.Oven_Vo = me->o.x;
	me->contr.Conveyor_D = me->c.d;
...
void top_syncInputEvents(top_t  *me) {
	...
	me->o.inputEvents.event.start = me->contr.outputEvents.event.Oven_Start; 
	
	me->o.inputEvents.event.remove = me->contr.outputEvents.event.Oven_Remove; 
	
	me->o.inputEvents.event.done = me->contr.outputEvents.event.Oven_Done; 
	
	me->c.inputEvents.event.off = me->contr.outputEvents.event.Conveyor_Off; 
	
	me->c.inputEvents.event.on = me->contr.outputEvents.event.Conveyor_On; 
	
	me->contr.inputEvents.event.update = me->o.outputEvents.event.xChange || me->c.outputEvents.event.xdchange; 
...
void top_syncInputData(top_t  *me) {
	...
	//sync for o (of type oven) which is a BFB
	
	
	//sync for c (of type conveyor) which is a BFB
	
	
	//sync for contr (of type controller) which is a BFB
	
	if(me->contr.inputEvents.event.update == 1) { 
		me->contr.Conveyor_D = me->c.d;
		me->contr.Oven_Vo = me->o.x;
	} 
