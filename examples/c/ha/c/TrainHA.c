...
void TrainHA_fast_mode_setup_0_algo(TrainHA_t  *me) {
	...
	NV_Ith_S(me->ode_solution, x);
	me->T0 = 0; //????

	//initialize solver with pointer to values
	flag = CVodeInit(me->cvode_mem, fast_mode_algo, me->T0, me->ode_solution);
    if (flag < 0) {
...
void TrainHA_slow_mode_2_setup_0_algo(TrainHA_t  *me) {
	...
	//initialize solver with pointer to values
	flag = CVodeInit(me->cvode_mem, slow_mode_2_algo, me->T0, me->ode_solution);
    if (flag < 0) {
		fprintf(stderr, "Error in CVodeMalloc: %d\n", flag);
		while(1);
    }
...
void TrainHA_slow_mode_1_setup_0_algo(TrainHA_t  *me) {
	...
	//initialize solver with pointer to values
	flag = CVodeInit(me->cvode_mem, slow_mode_1_algo, me->T0, me->ode_solution);
    if (flag < 0) {
		fprintf(stderr, "Error in CVodeMalloc: %d\n", flag);
		while(1);
    }
