...
void task0(_Core102_t * c0);
void task1(_Core102_t * c1);
void task2(_Core102_t * c2);
void task3(_Core102_t * c3);


int main() {
	printf("messagepasser_mem tcrest4 startup.\n");
	printf("sizes: %lu, %lu, %lu, %lu\n", sizeof(_Core102_t), sizeof(_Core102_t), sizeof(_Core102_t), sizeof(_Core102_t));
	mp_init();
...
void __attribute__ ((noinline)) timed_task0(_Core102_t * c0) {
...
void __attribute__ ((noinline)) timed_task1(_Core102_t * c1) {
...
void __attribute__ ((noinline)) timed_task2(_Core102_t * c2) {
...
void __attribute__ ((noinline)) timed_task3(_Core102_t * c3) {
...
void t0(void* param) {
	...
	if(_Core102_preinit(c0) != 0 || _Core102_init(c0) != 0) {
		HEX = 15;
		return;
	}
...
void t1(void* param) {
	...
	if(_Core102_preinit(c1) != 0 || _Core102_init(c1) != 0) {
		HEX = 15;
		return;
	}
...
void t2(void* param) {
	...
	if(_Core102_preinit(c2) != 0 || _Core102_init(c2) != 0) {
		HEX = 15;
		return;
	}
...
void t3(void* param) {
	...
	if(_Core102_preinit(c3) != 0 || _Core102_init(c3) != 0) {
		HEX = 15;
		return;
	}
