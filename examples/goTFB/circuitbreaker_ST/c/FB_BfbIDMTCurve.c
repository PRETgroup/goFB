...
int BfbIDMTCurve_preinit(BfbIDMTCurve_t  *me) {
	...
	me->_state = STATE_BfbIDMTCurve_s_init;
	me->_trigger = true;
...
void BfbIDMTCurve_s_safe_alg0(BfbIDMTCurve_t  *me) {
//PROVIDED CODE: this algorithm was provided in an algorithm's text field
me->cnt = 0;

}

void BfbIDMTCurve_s_count_alg0(BfbIDMTCurve_t  *me) {
//PROVIDED CODE: this algorithm was provided in an algorithm's text field
me->cnt = me->cnt + 1;
...
void BfbIDMTCurve_run(BfbIDMTCurve_t  *me) {
	...
	if(me->_trigger == false) {
		switch(me->_state) {
		case STATE_BfbIDMTCurve_s_init:
			if(true) {
				
				me->_state = STATE_BfbIDMTCurve_s_safe;
				me->_trigger = true;
			};
			break;
		case STATE_BfbIDMTCurve_s_safe:
			if(me->inputEvents.event.tick && me->i > me->iSet) {
				
				me->_state = STATE_BfbIDMTCurve_s_count;
				me->_trigger = true;
			};
			break;
		case STATE_BfbIDMTCurve_s_count:
			if(me->inputEvents.event.tick && me->i <= me->iSet) {
				
				me->_state = STATE_BfbIDMTCurve_s_safe;
				me->_trigger = true;
			} else if(me->inputEvents.event.tick && me->cnt > me->max) {
				
				me->_state = STATE_BfbIDMTCurve_s_unsafe;
				me->_trigger = true;
			...
			break;
		case STATE_BfbIDMTCurve_s_unsafe:
			if(me->inputEvents.event.tick && me->i <= me->iSet) {
	...
	if(me->_trigger == true) {
		...
		case STATE_BfbIDMTCurve_s_init:
			#ifdef PRINT_STATES
				printf("BfbIDMTCurve: [Entered State s_init]\n");
			#endif
			
			break;
		case STATE_BfbIDMTCurve_s_safe:
			#ifdef PRINT_STATES
				printf("BfbIDMTCurve: [Entered State s_safe]\n");
			#endif
		...
		case STATE_BfbIDMTCurve_s_count:
			#ifdef PRINT_STATES
				printf("BfbIDMTCurve: [Entered State s_count]\n");
			#endif
		...
		case STATE_BfbIDMTCurve_s_unsafe:
			#ifdef PRINT_STATES
				printf("BfbIDMTCurve: [Entered State s_unsafe]\n");
			#endif
