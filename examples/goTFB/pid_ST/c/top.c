...
//This is the main file for the iec61499 network with PIDController as the top level block
...
int main() {
	if(PIDController_preinit(&PIDController) != 0) {
		printf("Failed to preinitialize.");
		return 1;
	}
	if(PIDController_init(&PIDController) != 0) {
...
//this is executing with synchronous semantics
	//printf("\n\n\n");
	//printf("\nTop: %20s   Size: %lu\n", "PIDController", sizeof(myPIDController));
	...
	do {
		...
		PIDController_run(&PIDController);
		#ifdef PRINT_VALS
			printf("\n");
		#endif
	} 
