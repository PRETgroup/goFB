...
// This file represents the implementation of the ODE / Hybrid Automata Function Block for TrainHA
#include "TrainHA.h"
...
static int TrainHA_fast_mode_algo_ode_f(realtype t, N_Vector ode_solution, N_Vector ode_solution_dot, void *user_data) {
	//AUTOGENERATED CODE: this function represents the ODE in TrainHA's fast_mode_algo algorithm
	//it is in a format that CVODE can process.
	TrainHA_t *me = (TrainHA_t*)user_data;
...
static int TrainHA_fast_mode_algo_ode_g(realtype t, N_Vector ode_solution, realtype *gout, void *user_data) {
	//AUTOGENERATED CODE: this function represents the roots of the ODE in TrainHA's fast_mode_algo algorithm
	//it is in a format that CVODE can process.
	
	//I don't think I'll ever need this: TrainHA_t *me = (TrainHA_t*)user_data;
	
	
	gout[0] = NV_Ith_S(ode_solution, 0);
	
	return 0;
}

void TrainHA_fast_mode_algo(TrainHA_t *me) {
	//AUTOGENERATED CODE: this function manages iterations of the ODE in TrainHA's fast_mode_algo algorithm
...
static int TrainHA_slow_mode_1_algo_ode_f(realtype t, N_Vector ode_solution, N_Vector ode_solution_dot, void *user_data) {
	//AUTOGENERATED CODE: this function represents the ODE in TrainHA's slow_mode_1_algo algorithm
	//it is in a format that CVODE can process.
	TrainHA_t *me = (TrainHA_t*)user_data;
...
static int TrainHA_slow_mode_1_algo_ode_g(realtype t, N_Vector ode_solution, realtype *gout, void *user_data) {
	//AUTOGENERATED CODE: this function represents the roots of the ODE in TrainHA's slow_mode_1_algo algorithm
	//it is in a format that CVODE can process.
...
void TrainHA_slow_mode_1_algo(TrainHA_t *me) {
	//AUTOGENERATED CODE: this function manages iterations of the ODE in TrainHA's slow_mode_1_algo algorithm
	//It calls the CVODE solver and sets the outputs of the HA FB.
	me->Tnext += me->delta;
	int flag = CVode(me->cvode_mem, me->Tnext, me->ode_solution, &me->Tcurr, CV_NORMAL);
	if (flag < 0) {
...
static int TrainHA_slow_mode_2_algo_ode_f(realtype t, N_Vector ode_solution, N_Vector ode_solution_dot, void *user_data) {
	//AUTOGENERATED CODE: this function represents the ODE in TrainHA's slow_mode_2_algo algorithm
	//it is in a format that CVODE can process.
	TrainHA_t *me = (TrainHA_t*)user_data;
...
static int TrainHA_slow_mode_2_algo_ode_g(realtype t, N_Vector ode_solution, realtype *gout, void *user_data) {
	//AUTOGENERATED CODE: this function represents the roots of the ODE in TrainHA's slow_mode_2_algo algorithm
	//it is in a format that CVODE can process.
...
void TrainHA_slow_mode_2_algo(TrainHA_t *me) {
	//AUTOGENERATED CODE: this function manages iterations of the ODE in TrainHA's slow_mode_2_algo algorithm
	//It calls the CVODE solver and sets the outputs of the HA FB.
	me->Tnext += me->delta;
	int flag = CVode(me->cvode_mem, me->Tnext, me->ode_solution, &me->Tcurr, CV_NORMAL);
	if (flag < 0) {
...
void TrainHA_fast_mode_setup_0_algo(TrainHA_t *me) {
	...
	flag = CVodeInit(me->cvode_mem, TrainHA_fast_mode_algo_ode_f, me->T0, me->ode_solution);
    if (flag < 0) {
	...
	//set root function
	flag = CVodeRootInit(me->cvode_mem, 1, TrainHA_fast_mode_algo_ode_g);
	if (flag < 0) {
...
void TrainHA_slow_mode_2_setup_0_algo(TrainHA_t *me) {
	...
	//initialize solver with pointer to values
	flag = CVodeInit(me->cvode_mem, TrainHA_slow_mode_2_algo_ode_f, me->T0, me->ode_solution);
    if (flag < 0) {
		fprintf(stderr, "Error in CVodeMalloc: %d\n", flag);
		while(1);
    }
	...
	//set root function
	flag = CVodeRootInit(me->cvode_mem, 1, TrainHA_slow_mode_2_algo_ode_g);
	if (flag < 0) {
		fprintf(stderr, "Error in CVodeRootInit: %d\n", flag);
		while(1);
	}
...
void TrainHA_slow_mode_1_setup_0_algo(TrainHA_t *me) {
	...
	//initialize solver with pointer to values
	flag = CVodeInit(me->cvode_mem, TrainHA_slow_mode_1_algo_ode_f, me->T0, me->ode_solution);
    if (flag < 0) {
		fprintf(stderr, "Error in CVodeMalloc: %d\n", flag);
		while(1);
    }
	...
	//set root function
	flag = CVodeRootInit(me->cvode_mem, 1, TrainHA_slow_mode_1_algo_ode_g);
	if (flag < 0) {
		fprintf(stderr, "Error in CVodeRootInit: %d\n", flag);
		while(1);
	}
