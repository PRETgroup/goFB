...
int top_preinit(top_t  *me) {
	...
	if(TickGen_preinit(&me->g) != 0) {
		return 1;
	}
	if(TrainHA_preinit(&me->ha) != 0) {
		return 1;
	}
	if(Output_preinit(&me->p) != 0) {
		return 1;
	}
...
int top_init(top_t  *me) {
	...
	me->ha.delta = me->g.delta;
	me->p.var = me->ha.x;
	

	//if there are fb children (CFBs/Devices/Resources only), call this same function on them
	if(TickGen_init(&me->g) != 0) {
		return 1;
	}
	if(TrainHA_init(&me->ha) != 0) {
		return 1;
	}
	if(Output_init(&me->p) != 0) {
		return 1;
	}
...
void top_syncInputEvents(top_t  *me) {
	...
	me->ha.inputEvents.event.tick = me->g.outputEvents.event.tick; 
	
	me->p.inputEvents.event.output = me->ha.outputEvents.event.update; 
...
void top_syncInputData(top_t  *me) {
	...
	//sync for g (of type TickGen) which is a BFB
	
	
	//sync for ha (of type TrainHA) which is a BFB
	
	if(me->ha.inputEvents.event.tick == 1) { 
		me->ha.delta = me->g.delta;
	} 
	
	//sync for p (of type Output) which is a BFB
	
	if(me->p.inputEvents.event.output == 1) { 
		me->p.var = me->ha.x;
	} 
