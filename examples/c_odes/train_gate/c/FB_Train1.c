...
// This file represents the implementation of the ODE / Hybrid Automata Function Block for Train1
#include "FB_Train1.h"


/* Train1_preinit() is required to be called to 
 * initialise an instance of Train1. 
...
/* Train1_init() is required to be called to 
 * set up an instance of Train1. 
...
static int Train1_yDotEq1_ode_f(realtype t, N_Vector ode_solution, N_Vector ode_solution_dot, void *user_data) {
	//AUTOGENERATED CODE: this function represents the ODE in Train1's yDotEq1 algorithm
	//it is in a format that CVODE can process.
	Train1_t *me = (Train1_t*)user_data;
...
static int Train1_yDotEq1_ode_g(realtype t, N_Vector ode_solution, realtype *gout, void *user_data) {
	//AUTOGENERATED CODE: this function represents the roots of the ODE in Train1's yDotEq1 algorithm
	//it is in a format that CVODE can process.
...
int Train1_yDotEq1(Train1_t *me) {
	//AUTOGENERATED CODE: this function manages iterations of the ODE in Train1's yDotEq1 algorithm
...
static int Train1_yDotEq2_ode_f(realtype t, N_Vector ode_solution, N_Vector ode_solution_dot, void *user_data) {
	//AUTOGENERATED CODE: this function represents the ODE in Train1's yDotEq2 algorithm
	//it is in a format that CVODE can process.
	Train1_t *me = (Train1_t*)user_data;
...
static int Train1_yDotEq2_ode_g(realtype t, N_Vector ode_solution, realtype *gout, void *user_data) {
	//AUTOGENERATED CODE: this function represents the roots of the ODE in Train1's yDotEq2 algorithm
	//it is in a format that CVODE can process.
...
//return 1 if we found a root
int Train1_yDotEq2(Train1_t *me) {
	//AUTOGENERATED CODE: this function manages iterations of the ODE in Train1's yDotEq2 algorithm
	//It calls the CVODE solver and sets the outputs of the HA FB.
	if(me->solveInProgress == 0) {
		me->Tnext += me->DeltaTime;
		me->solveInProgress = 1; //solveInProgress is used to mark if we are currently solving
...
void Train1_yPrimeEq0(Train1_t *me, CVRhsFn ode_f, CVRootFn ode_g) {
...
void Train1_yPrimeEqy(Train1_t *me, CVRhsFn ode_f, CVRootFn ode_g) {
	//AUTOGENERATED CODE: this algorithm specifies the initialization of a CVODE SUNDIALS solver and was parsed from the algorithm's text field
	int flag;
...
/* Train1_run() executes a single tick of an
 * instance of Train1 according to synchronous semantics.
...
repeat: 	//when we have had a mid-tick transition, we want to start the run again 
	...
	if(me->_trigger == false) {
		switch(me->_state) {
		case STATE_Train1_Start:
			if(true) {
				
				me->_state = STATE_Train1_State1E0;
				me->_trigger = true;
			};
			break;
		case STATE_Train1_State1E0:
			if(true) {
	...
	if(me->_trigger == true) {
		...
		case STATE_Train1_State1E0:
			...
			Train1_yPrimeEq0(me, Train1_yDotEq1_ode_f, Train1_yDotEq1_ode_g);
			me->outputEvents.event.update_t = 1;
			
			odeRootFound = Train1_yDotEq1(me);
		...
		case STATE_Train1_State2E0:
			...
			//init all ode algorithms that this state feeds into
			Train1_yPrimeEqy(me, Train1_yDotEq2_ode_f, Train1_yDotEq2_ode_g);
			me->outputEvents.event.update_t = 1;
			
			odeRootFound = Train1_yDotEq2(me);
		...
		case STATE_Train1_State3E0:
			...
			//init all ode algorithms that this state feeds into
			Train1_yPrimeEqy(me, Train1_yDotEq1_ode_f, Train1_yDotEq1_ode_g);
			me->outputEvents.event.update_t = 1;
